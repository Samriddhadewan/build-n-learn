export default function Home() {
    const logos = [
        ...
        },
        {
            name: "Gemini",
            circle: "circle-5",
            icon: <MetaIconOutline className="size-4" />
        },
    ]
    return (
        <div className="relative h-full w-full flex items-center justify-center">
            <div className="z-10 h-80 w-60 bg-neutral-50 shadow-2xl border border-neutral-100 rounded-xl overflow-hidden">
                <div className="relative mask-r-from-50% mask-l-from-50%">
                    <Pattern />
                    <div className="flex items-center h-40 gap-4 justify-center   animate-marquee">
                        {
                            logos.map((item, index) => <div className={`flex items-center justify-center rounded-full bg-neutral-300 size-10`} key={item.name}>{item.icon}</div>)
                        }
                    </div>
                </div>
                <div className="p-4">
                    <h1 className="font-bold text-neutral-600 text-[8px]">This LLMs are getting out of hand</h1>
                    <p className="text-neutral-400 text-xs">
                        I swear to god there&apos;s always AI dropping and twitter goes crazy. It&apos;s all a myth guys
                    </p>
                </div>
...
const Pattern = () => {
    return (<div className="  
            absolute inset-0 z-0 rounded-lg m-auto bg-[repeating-linear-gradient(315deg,var(--pattern-fg)_0,var(--pattern-fg)_1px,transparent_0,transparent_50%)] bg-size-[10px_10px] bg-fixed"
    ></div>)
}
